...
function get_empfohlen_form_register($redirect=false) {
  ...
  if (!is_user_logged_in()){  
    ...
    if ( !empty($success) ){
      if(is_array($success)){
        foreach ($success as $succ) { $return .= "<p class=\"success\">{$succ}</p>\r\n"; }
      }else{
        $return .= "<p class=\"success\">{$success}</p>\r\n";
      }
    }
    ...
    if ( !empty($error) ){
      if(is_array($error)){
        foreach ($error as $err) {  $return .= "<p class=\"error\">{$err}</p>\r\n";  }
      }else{
         $return .= "<p class=\"error\">{$error}</p>\r\n";
      }
    }
    ...
    $return .= '        <input id="emp_user_first" name="emp_user_first"   type="text">';
    $return .= '    </div>';
    ...
    $return .= '        <input name="emp_user_login" id="emp_user_login" class="required" type="text"/>';
    $return .= '    </div>';
    $return .= '    <div class="two-right">';
    $return .= '        <label for="emp_user_email">'.__('Email', 'emp').'</label><br>';
    $return .= '        <input name="emp_user_email" id="emp_user_email" class="required" type="email"/>';
    $return .= '    </div>';
    $return .= '</div>';
    ...
    $return .= '        <input name="emp_user_pass" id="password" class="required" type="password"/>';
    $return .= '    </div>';
    $return .= '    <div class="two-right">';
    $return .= '        <label for="password_again">'.__('Password Again', 'emp').'</label><br>';
    $return .= '        <input name="emp_user_pass_confirm" id="password_again" class="required" type="password"/>';
    $return .= '    </div>';
    $return .= '</div>';
